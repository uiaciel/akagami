        ...
        <div class="form-group row mb-0">
            ...
            <div class="col-sm-7">
                ...
                <select class="form-select" name="passport_id" id="passport">
                    @foreach ($documents->where('crew_id', $crew->id)->where('type', 'Passport') as $passport)
                        <option value="{{ $passport->id }}">{{ $passport->no }}</option>
                    @endforeach
...
<div class="row">
        ...
        <div class="form-group row mb-0">
            ...
            <div class="col-sm-8">
                <div class="input-group mb-0">
                    ...
                    <label style="margin-left: 14px;margin-right: 7px;width: 16%;" class="fs-5">年齢<br>Age</label>
    ...
    <div class="col-3">
        <div class="form-group row mb-0">
            ...
            <div class="col-sm-7">
                <input type="text" class="form-control" id="passport_place" value="{{ $crew->passport_place }}">
            </div>
            ...
            <div class="col-sm-7">
                <select class="form-select" id="seaman">
                    @foreach ($documents->where('crew_id', $crew->id)->where('type', 'Seaman Book') as $seaman)
                        <option value="{{ $seaman->id }}">{{ $seaman->no }}</option>
                    @endforeach
                </select>
            </div>
            <label class="col-sm-5">発給日<br>Issued Date</label>
            ...
            <label for="" class="col-sm-5">発給地<br>Issued Place</label>
            <div class="col-sm-7">
                <input type="text" class="form-control" id="seaman_entry" value="{{ $crew->seamanbook_place }}">
            </div>
            <label for="" class="col-sm-5">有効期限<br>Valid Until</label>
            <div class="col-sm-7">
        ...
        <div class="form-group row mb-0">
            ...
            <div class="col-sm-7">
                <input type="text" class="form-control"
                    value="{{ $experiences->where('crew_id', $crew->id)->pluck('vesselsname')->first() }}">
            </div>
...
">
    ...
    <div class="col-2">
        <img id="imgPreview" src="/storage/{{ $crew->photo }}"
            src="https://komisi-kejaksaan.go.id/komjak/wp-content/uploads/2017/01/no-profile-photo.jpg" alt="..."
        ...
        <div class="form-group row mb-0">
            ...
            <div class="col-sm-8">
                ...
                <select class="form-select" id="vissa">
                    @foreach ($documents->where('crew_id', $crew->id)->where('type', 'Entry Visa') as $visa)
                        <option value="{{ $visa->id }}">{{ $visa->no }}</option>
                    @endforeach
                </select>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-7">
                <select class="form-select" id="orange">
                    @foreach ($documents->where('crew_id', $crew->id)->where('type', 'Seaman Book') as $orange)
                        <option value="{{ $orange->id }}">{{ $orange->no }}</option>
                    @endforeach
                </select>
            </div>
            <label class="col-sm-5">発給日<br>Issued Date</label>
            ...
            <label for="" class="col-sm-5">発給地<br>Issued Place</label>
            <div class="col-sm-7">
                <input type="text" class="form-control" id="orange_entry" value="{{ $crew->orangebook_place }}">
            </div>
            <label for="" class="col-sm-5">有効期限<br>Valid Until</label>
            <div class="col-sm-7">
...
<div class="row mt-3">
    <span>{{ \Carbon\Carbon::parse($experiences->where('crew_id', $crew->id)->pluck('signon')->first())->format('Y') }}
        年以降の乗船経歴</span>
    <h5>Experience since
        {{ \Carbon\Carbon::parse($experiences->where('crew_id', $crew->id)->pluck('signon')->first())->format('Y') }}
    </h5>
    <table class="table tableexp text-nowrap p-0">
        ...
        <tbody>
            @foreach ($experiences->where('crew_id', $crew->id) as $exp)
                <tr>
